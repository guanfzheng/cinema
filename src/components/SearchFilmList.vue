...
<script>
    ...
    created(){
    	this.showFilmList({name: this.$route.query.name,pageStart: this.paging.pageStart,pageSize:this.paging.pageSize});
    ...
    methods: {
		...
		async showFilmList(searchName){
    		const {data: res} = await this.$http.post("searchbyname",searchName);
